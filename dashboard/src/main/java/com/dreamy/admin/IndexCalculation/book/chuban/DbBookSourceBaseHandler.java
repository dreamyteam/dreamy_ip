 ...
 */
@Component
public class DbBookSourceBaseHandler extends ChubanBookSourceBaseHandler {

    @Autowired
    private BookScoreService bookScoreService;

    private CrawlerSourceEnums crawlerSourceEnums = CrawlerSourceEnums.douban;


    @Override
    public Integer getHandlerId() {
        return ChubanBookDataSourceEnums.douban.getSource();
    }
    ...
    public Integer getHotIndex(BookView bookView) {
        ...
        if (bookScore != null) {
            ...
            if (commentNum != null && commentNum > 0) {
                Double tmp = commentNum * ChubanBookHotIndexExchangeEnums.douban.getNum();
                score = tmp.intValue();
            }
        }
